...
export const Searchbar: React.FunctionComponent<PropTypes> = ({ setValue, value, openMenuItem }: PropTypes) => {
  ...
  return (
    <>
      <div className={`w-full text-gray-600 self-start  scrollbar-hide  ${searchbarOpen ? `flex justify-center overflow-y-auto h-full w-full left-0 fixed bottom-0 bgtrans no-scrollbar` : ``} `} onClick={() => setsearchbarOpen(false)}>
      </div >
      <div className='scrollbar-hide pt-5'>
        ...
        <div className={`text-gray-600 self-start flex bg-light-grey rounded-lg h-8 pl-3 ${searchbarOpen ? `sticky` : ``}  z-30`}>
          ...
          {/* Searchbar object */}
          <input onFocus={(e) => setsearchbarOpen(true)} id="searchbar" className="rounded-lg w-full  bg-light-grey text-sm text-grey focus:outline-none" placeholder="Search..." onChange={event => { handleChange(event.target.value) }}></input>
        </div>
      </div>

      <div className='pt-3 scrollbar-hide mr-32 '>
        <div className={` h-3/5 w-full self-start scrollbar-hide ${searchbarOpen ? `fixed` : ``}  rounded-lg z-30 overflow-y-scroll bg-white`}>
          {searchbarOpen ?
