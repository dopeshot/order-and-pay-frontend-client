    ...
    return (
        <>
            {categoryAndDishes.categories.length > 0 && categoryAndDishes.categories.map((category, index) => (
                <div key={category._id + index} data-cy={`section-${index}`} id={`section-${index}`} className={`pt-2`} ref={sectionRefs.current[index]}>
                    {/* Category banner */}
                    ...
                    <div className="px-5 divide-y divide-dividergrey">
                        {category.dishes.map((dish, dIndex) => (
                            <div key={dish._id} id={index + "_dishCard_Id"} data-cy={`cat-${index}_dish-${dIndex}`} className="block pb-2 pt-2 dish" onClick={() => openMenuItem(dish, category)} >
