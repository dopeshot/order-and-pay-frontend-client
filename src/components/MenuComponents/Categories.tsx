...
export const Categories: React.FunctionComponent = () => {
    ...
    const categories = state.menu.categories.map((category,index) => (
        <button key={category._id + "_button"} id={"categoryButton_" + index} className="cats text-red font-sofia font-bold text-center m-1 h-16 w-20 shadow-md  rounded-md overflow-hidden text-xs b-2 focus:bg-red focus:text-white" onClick={() => scrollTo(category._id)} >
