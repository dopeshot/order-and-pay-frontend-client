...
export const ScrollCats: React.FunctionComponent<PropTypes> = (props: PropTypes) => {
    ...
    const categories = state.menu.categories.map((category,index) => (
        <button key={category._id + "_button"} id={"categoryScroll_" + index} className="text-red font-sofia font-bold text-center m-1 h-10 w-20 shadow-md rounded-md overflow-hidden text-xs b-2 focus:bg-red focus:text-white" onClick={() => props.scrollFC(index)}  >
