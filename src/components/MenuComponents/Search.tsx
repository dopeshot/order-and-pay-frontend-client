...
export const Search: React.FunctionComponent<PropTypes> = ({ setValue }: PropTypes) => {
    ...
    return (
        <div className="w-full text-gray-600 self-start flex bg-light-grey rounded-lg h-8 pl-3">
            ...
            {/* Searchbar object */}
            <input autoFocus id="searchbar2" data-cy="searchbar2" className="w-full bg-transparent text-sm text-grey focus:outline-none " placeholder="Suchen..." onChange={event => { handleChange(event.target.value) }}></input>
        </div>
    )
}